...
6. 7/10: restituzioni al pool (cron), lead in `/import` bucket `LANCIO_WEBDEV_2026`; `offerta_del_mese_link` per il pulsante dell'agenda GDO. Il 6/10 e' tutto del bot: nessuna restituzione. Si restituisce chi non ha mai risposto e chi ha la chat ferma da piu' di 24 ore (ultimo messaggio in qualunque direzione); chi e' in conversazione viva resta al bot e torna al pool man mano.
